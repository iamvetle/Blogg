    ...
    >
      <div id="editor-area">
        ...
        >
          ...
          <button @click="editor.chain().focus().toggleCodeblock().run()" :class="{ 'is-active': editor.isActive('code') }">
            <img
              class="h-5 flex items-center"
              :src="code_snip_icon"
              alt="code"
...
const html2 = ref(null)

onMounted(() => {
editor.value.on("update", () => {
html2.value = editor.value.gethtml()
})
...
const newPostMaterial = async () => {
	...
	const { title, body } = extractTitleAndContent(html2.value)
