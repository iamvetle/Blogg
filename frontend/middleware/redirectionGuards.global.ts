		...
		if (authStore.isAuthenticated === false) {
			...
			if (process.client) {
				...
				if (authStore.isAuthenticated) {
					...
					console.log("CONTINUE: The store - AFTER a check (withing client side) - says that the user is NOT authenticated (false):", authStore.isAuthenticated) // print to self
				}
			}
		} else {
		...
		if (authStore.isAuthenticated === false) {
			...
			if (process.client) {
				...
				if (authStore.isAuthenticated === false) {
					...
					console.log("CONTINUE: The store - AFTER a check (withing client side) - says that the user IS authenticated (true):", authStore.isAuthenticated) // print to self
				}
			}
		} else {
			console.log("REDIRECTED: The store said that the user was authenticated - so there is NO NEED to go 'client-side' to check further (true). So redirected to ->", redirectPath, authStore.isAuthenticated) // print to self
